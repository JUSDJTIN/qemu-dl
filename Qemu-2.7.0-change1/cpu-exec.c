...
void ShadowStackInit(void){
    ...
    pss -> top = 0 ;
    pss -> MaxSize = 50;
    pss -> stack = (target_ulong *)malloc(50 * sizeof(target_ulong));
    if (!ss->stack){
        printf("[info]:init shadow stack faild !!!!!!!!!!!!!!!!!!!!!!");
    }
}
void ShadowStackPush(target_ulong x){
    ShadowStack * pss;
    pss = &ss1;
    if (pss->top >= pss-> MaxSize)
        {
            pss->stack = (target_ulong*)relloc(pss->stack,2*pss->MaxSize*sizeof(target_ulong));
            pss->MaxSize = 2 * pss->MaxSize;
        }
    pss->stack[ss->top] = x;
    pss->top += 1;
}
